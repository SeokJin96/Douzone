...
/*
 ...
 클래스 == 설계도 == 데이터 타입
 클래스의 종류
 1. class Car {} > main 함수를 갖고 있지 않은 클래스 > 독자적 실행이 불가능 > 다른 클래스에서 도움을 주는 클래스(lib)
...
1. instance variable : 객체변수(클래스 안)  >> class Car { public int door;}
2. local variable : 지역변수(함수 안) >> class Car { public void run() { int data=0; } }
3. static variable : 공유자원(객체간
) >>class Car {public static in num;}
